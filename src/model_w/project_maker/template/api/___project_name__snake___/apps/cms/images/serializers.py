"""Custom serializers for image handling in the CMS app."""
...
class ImageSerializer(ReadOnlyField):
    ...
    def __get_sources(
        ...
        If the file is a gif, the renditions will be returned as a list of gifs to preserve animation.
        """
        if instance.file.name.endswith(".gif"):
            self.formats = ["gif"]
    ...
    def to_representation(self, instance: wagtail_images_models.AbstractImage):
        ...
        Note: SVGs are not rasterised, so we don't return sources, just the URL.
              In the front, this can be used as normal, or render it as HTML.
        """
        data = None

        if instance:
            is_svg = instance.file.name.endswith(".svg")
            ...
            data = {
                ...
                "sources": [] if is_svg else self.__get_sources(instance),
            }
        return data
